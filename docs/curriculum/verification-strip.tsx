import { type CheckKind, CHECK_LABEL } from '@/lib/tracks';
 ...
 * recolour these for aesthetic reasons; they carry meaning.
 */
...
export function VerificationStrip({
  ...
  return (
    <div
      className={`flex ${height} overflow-hidden rounded-full bg-neutral-200 ${className}`}
      role="img"
      aria-label={kinds
        .filter((k) => mix[k] > 0)
      ...
      {kinds.map((k) =>
        mix[k] > 0 ? (
          <li key={k} className="flex items-center gap-1.5">
            ...
            {CHECK_LABEL[k]}
            <span className="font-mono tabular-nums text-neutral-400">{mix[k]}</span>
          </li>
        ) : null,
      )}
    </ul>
