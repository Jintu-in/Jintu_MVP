/**
 * Emits version 3 of the Data Analyst curriculum: the verified tier earned.
 *
 * The one change that matters: week 3 returns to the seeded planted-defect
 * export — the only artifact gradeable today with no SQL runner and no
 * model — and the prose rubrics gain the structural checks a machine can
 * hold (sections present, link opens). Machine share moves 28% -> 58%,
 * which is what lets tracks.tier = 'verified' tell the truth.
 *
 ...
 *
 * So this builds path version 3 as a draft, fills in all six weeks, and
 ...
 *
 * Re-running is safe. Every insert is guarded, and if version 3 is already
...
const RUBRICS = [
  ...
  // New in v3: the defect audit — every point machine-checked against the
  // ops-held key. The display split (find / count / no fabrication) is the
  // promise; the checker nets fabrications against hits (the decoy penalty),
  // so the third row is where careless box-ticking loses its point.
  {
    name: "defect-audit-v1",
    max_score: 7,
    criteria: [
      { key: "found_classes", label: "Finds at least three of the planted problem classes", weight: 5, check: "detectable", checker: "answer_key_match:3" },
      { key: "counts_hold", label: "Every claimed count matches the data", weight: 1, check: "detectable", checker: "answer_key_match:3" },
      { key: "no_fabrication", label: "No claimed problem that is not in the data", weight: 1, check: "detectable", checker: "answer_key_match:3" },
    ],
  },
  {
    name: "written-finding-v2",
    max_score: 7,
    criteria: [
      { key: "has_sections", label: "Finding, Confidence and Caveats sections present", weight: 1, check: "structural", checker: "has_sections:Finding,Confidence,Caveats" },
  ...
  },
  {
    ...
    max_score: 5,
    criteria: [
      { key: "readable_link", label: "The recording link opens without signing in", weight: 1, check: "structural", checker: "url_reachable" },
      { key: "under_five_minutes", label: "Makes the point in under five minutes", weight: 2, check: "peer", checker: null },
      { key: "explains_why", label: "Explains a decision, not only what was built", weight: 2, check: "peer", checker: null },
    ],
  },
  {
    name: "dashboard-clarity-v2",
    max_score: 7,
    criteria: [
      { key: "readable_link", label: "The dashboard opens without signing in", weight: 1, check: "structural", checker: "url_reachable" },
      ...
      { key: "traceable", label: "Every number names the query or table behind it", weight: 1, check: "peer", checker: null },
    ],
  },
];
...
const WEEKS = [
  ...
  },
  {
    ...
    resources: [
      ...
      [
        "dataset",
        "https://nejtogeezerebdeflhfa.supabase.co/storage/v1/object/public/datasets/defects-da3.csv",
        "The orders export you audit — 2,000 rows, problems included",
      ],
      [
        "dataset",
        "https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-Present/erm2-nwe9",
        "NYC 311 service requests — optional extra practice on a truly wild dataset",
      ],
    ],
    assignment: {
      kind: "artifact_link",
      rubric: "defect-audit-v1",
      weight: 1,
      spec: {
        prompt:
          "Audit the orders export linked in this week's resources: 2,000 rows " +
          "from a company that does not know what is wrong with its data. Write " +
          "a one-page audit and, below it, tick every problem class you actually " +
          "found. For each, your page must say how many rows it affects — an " +
          "analyst who says 'the data is messy' has said nothing; the number is " +
          "the finding. Some offered codes are not in the data at all, and " +
          "claiming one counts against you. The README in the export names what " +
          "is legitimately blank or zero — read it before crying wolf.",
        codes: [
          "amount-string-total", "currency-mixed", "delivered-before-ordered",
          "duplicate-rows", "future-date", "impossible-age", "mixed-case-category",
          "negative-amount", "null-order-id", "orphan-customer", "whitespace-name",
        ],
      },
    },
  },
...
const REPS = {
  ...
  3: [
    [1, "Find every fully duplicated row in the orders export. Paste the query and the count.", ["non_empty"]],
    [2, "Count orders delivered before they were placed. Paste query and count.", ["non_empty"]],
    [3, "List the five messiest category values, and what you would map each to.", ["non_empty"]],
  ],
  4: [
...
const out = [];
out.push(`-- Data Analyst — first job: curriculum version 3: the verified tier earned.`);
out.push(`-- Generated by scripts/generate-data-analyst-v3.mjs. Do not edit by hand.`);
out.push(`--`);
out.push(`-- Safe to run repeatedly. Versions 1 and 2 are left untouched — they are published,`);
out.push(`-- and published paths are immutable by trigger, not by convention.`);
out.push("");

// sql-correctness-v1 predates the archetype fields (#57): the live row
// carries labels and weights but no check/checker, so anything computing
// the machine share from the DATA undercounts by the two sql weeks. Same
// keys, same weights, same labels — only the metadata is added, and only
// when it is missing, so finished work is never repriced.
const SQL_V1_WITH_CHECKS = JSON.stringify([
  { key: "returns_expected_rows", label: "Returns the expected result set", weight: 3, check: "executable", checker: "sql_diff" },
  { key: "no_cartesian", label: "No accidental cross join", weight: 1, check: "structural", checker: "sql_diff" },
  { key: "readable", label: "Aliases and formatting a reviewer can follow", weight: 1, check: "structural", checker: "sql_diff" },
]);
out.push(`update public.rubrics set criteria = ${q(SQL_V1_WITH_CHECKS)}::jsonb`);
out.push(`where name = 'sql-correctness-v1' and not (criteria->0 ? 'check');`);
out.push("");
...
out.push(`  values (v_track, 3, 'draft')`);
out.push(`  on conflict (track_id, version) do nothing;`);
out.push("");
out.push(`  select id, status into v_path, v_state`);
out.push(`  from public.paths where track_id = v_track and version = 3;`);
out.push("");
...
out.push(`  end if;`);
out.push("");
out.push(`  -- The margin, recorded: 21 of 36 rubric points are machine-checked
  -- (2×5 sql, 7 detectable, 2+1+1 structural on the prose weeks) = 58.3%.
  -- tracks_verified_needs_deterministic holds >= 0.50 from here on.
  update public.tracks set deterministic_share = 0.5833 where id = v_track;
