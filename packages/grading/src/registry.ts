import { gradeSqlSubmission } from "./deterministic/sql";
import type { QueryRunner, SqlAssignmentSpec } from "./deterministic/types";
...
/**
 * All eleven names, including the three that do not resolve yet. Track
 * validation checks names against THIS list, so an author can reference a
 * planned checker in a draft — but publish-time validation must require every
 * checker on a published track to be in CHECKERS, which only holds the
 * implemented ones.
 */
...
const answer_key_match: Checker = (input, args) => {
  ...
  const rec = asRecord(input);
  const found = Array.isArray(rec?.found) ? (rec.found as unknown[]).map(String) : null;
  const planted = Array.isArray(rec?.planted) ? (rec.planted as unknown[]).map(String) : null;
  if (!found || !planted) {
  ...
  if (hits < minHits) {
    return refuse(
      `Found ${hits} of the planted problems; this artifact needs ${minHits}. ` +
...
/**
 * Only the implemented eight. The other three names exist in CHECKER_NAMES
 ...
 *   media_has_audio  needs real media probing; a checker that trusts client
 *                    metadata would claim to verify the one thing it cannot
 *   rubric_score     the only checker that spends money — lands behind
 *                    budget_guards in Phase 2, never before
 *
 * Publish-time validation must require every checker named by a published
 * track to resolve HERE. A draft may reference the planned three; a paid
 * sprint may not promise a check nothing performs.
 */
